...
pub struct Diagnostic<'diag> {
    ...
    pub note: Option<DiagNote>,
    pub help: Option<DiagHelp>,
}

impl<'diag> Diagnostic<'diag> {
    pub fn new(diag_type: DiagType, code: Option<u16>, message: DiagMessage) -> Box<Self> {
        Box::new(Self {
            diag_type,
            code,
            lint_type: None,
    ...
    pub fn weak_warning(message: DiagMessage) -> Box<Self> {
        Self::new(DiagType::WeakWarning, None, message)
    }

    pub fn warning(message: DiagMessage) -> Box<Self> {
        Self::new(DiagType::Warning, None, message)
    }

    pub fn error(message: DiagMessage) -> Box<Self> {
        Self::new(DiagType::Error, None, message)
    }

    pub fn error_with_code(message: DiagMessage, code: u16) -> Box<Self> {
        Self::new(DiagType::Error, Some(code), message)
    }

    pub fn add_label(mut self: Box<Self>, label: DiagLabel) -> Box<Self> {
        self.labels.push(label);
        self
    }

    pub fn set_help(mut self: Box<Self>, help: DiagHelp) -> Box<Self> {
        self.help = Some(help);
        self
    }

    pub fn set_note(mut self: Box<Self>, note: DiagNote) -> Box<Self> {
        self.note = Some(note);
        self
    }
