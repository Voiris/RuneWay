...
impl<'src, 'diag> Lexer<'src> {
    ...
    fn duplicated_prefix_error(
        ...
        Diagnostic::error(DiagMessage::new(
            super::messages::DUPLICATED_STRING_LITERAL_PREFIX,
            &[("char", &prefix.to_string())],
        ))
        .add_label(DiagLabel::silent_primary(Span::new(lo, hi, self.source_id)))
    }
    ...
    fn invalid_escape_sequence_error(
        ...
        Diagnostic::error(DiagMessage::new(
            super::messages::INVALID_ESCAPE_SEQUENCE,
            &[("sequence", &sequence)],
        ))
        .add_label(DiagLabel::silent_primary(Span::new(lo, hi, self.source_id)))
    }
    ...
    fn handle_string_prefix(&mut self) -> LexerResult<'diag, (bool, bool)> {
        ...
        match (first, second) {
            ...
            (_, c) => {
                self.cursor.next();
                let hi = self.cursor.pos();
                Err(Diagnostic::error(DiagMessage::new(
                    super::messages::INVALID_CHAR,
                    &[("char", &c.to_string())],
                ))
                .add_label(DiagLabel::silent_primary(Span::new(
                    lo,
                    hi,
                    self.source_id,
                ))))
                ...
                'x' => {
                    ...
                    if let Some(hex_str) = hex_str_opt {
                        ...
                        if let Ok(hex) = hex_opt {
                            ...
                            let escape_hi = self.cursor.pos();
                            Err(self.invalid_escape_sequence_error(
                                escape_lo,
                                escape_hi,
                                r"\x".to_string(),
                            ))
                ...
                'u' => {
                    ...
                    if let Ok(hex) = hex_opt {
                        match hex {
                            ...
                            .add_label(DiagLabel::silent_primary(
                                span!(self.source_id => hex_lo..hex_hi),
                            ))
                            .set_help(DiagHelp::new(
                                super::messages::UNICODE_ESCAPE_MUST_NOT_BE_SURROGATE,
                                &[],
                            ))),
                            0x110000.. => Err(Diagnostic::error(DiagMessage::new(
                                super::messages::INVALID_UNICODE_ESCAPE,
                                &[],
                            ))
                            .add_label(DiagLabel::silent_primary(
                            ...
                            .set_help(DiagHelp::new(
                                super::messages::UNICODE_ESCAPE_MUST_BE_IN_RANGE,
                                &[],
                            ))),
                            hex => {
    ...
    fn lex_string_literal(
        ...
        is_raw: bool,
        is_format: bool,
        consume_starter_quote: bool,
    ) -> LexerResult<'diag, (SpannedToken<'src>, bool)> {
        let lo = self.cursor.pos();
            ...
            match char {
                ...
                '\\' if !is_raw => {
                    let string = string_opt.get_or_insert_with(|| {
                        self.source_file.src()[raw_str_lo.to_usize()..self.cursor.pos().to_usize()]
                            .to_string()
                    });
        ...
        if !is_terminated {
            ...
            .add_label(DiagLabel::silent_primary(span!(self.source_id => lo..hi))));
        }
        ...
        let token = if let Some(string) = string_opt {
            ...
            Token::RawStringLiteral(
                &self.source_file.src()[raw_str_lo.to_usize()..raw_str_hi.to_usize()],
            )
        };
    ...
    fn lex_string(
        ...
        is_raw: bool,
        is_format: bool,
    ) -> LexerResult<'diag, Vec<SpannedToken<'src>>> {
        let lo = self.cursor.pos();
        ...
        while let Some(char) = self.cursor.peek_char() {
            match char {
                '{' => {
                    ...
                    while let Some(char) = {
                        self.cursor.consume_while(|c| c.is_whitespace());
                        self.cursor.peek_char().cloned()
                    } {
                        match char {
                            '}' if brace_level == 1 => {
                                brace_level -= 1;
                ...
                _ => {
                    let (literal, is_quote_terminated) =
                        self.lex_string_literal(is_raw, is_format, false)?;

                    if is_quote_terminated {
                        // Not count ending quote
                        tokens.push(SpannedToken::new(
                            literal.node,
                            Span::new(literal.span.lo, literal.span.hi - 1, literal.span.src_id),
                        ));
                        is_terminated = true;
                        break;
                    } else {
    ...
    fn lex_char_literal(&mut self) -> LexerResult<'diag, SpannedToken<'src>> {
        ...
        let char = match self.cursor.peek_char() {
            ...
            Some('\'') => {
                ...
                .add_label(DiagLabel::simple_primary(
                    super::messages::EMPTY_CHAR_LITERAL,
                    span!(self.source_id => lo..hi),
                )));
            }
        ...
        while let Some(&char) = self.cursor.peek_char() {
            match char {
                ...
                'e' | 'E' if !is_exponent && radix == Radix::Decimal => {
                    match (self.cursor.lookahead_char(1), self.cursor.lookahead_char(2)) {
                        (Some('0'..='9'), _) => {
