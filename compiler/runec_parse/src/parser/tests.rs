...
fn attribute_access_expr<'src>(source_id: SourceId) -> SpannedExpr<'src> {
    SpannedExpr::new(
...
fn act_parse_test() {
    ...
    let expected_stmts = [SpannedStmt::new(
        Stmt::DefineFunction {
            ident: SpannedStr::new(
                "main",
                Span::new(BytePos::from_usize(4), BytePos::from_usize(8), source_id),
            ),
            args: Box::new([]),
            ret_ty: SpannedTypeAnnotation::new(
                TypeAnnotation::Unit,
                Span::new(BytePos::from_usize(10), BytePos::from_usize(10), source_id),
            ),
        ...
        Stmt::DefineFunction {
            ...
            args: Box::new([
                FunctionArg {
                    ...
                    ty: SpannedTypeAnnotation::new(
                        TypeAnnotation::Ident("b"),
                        Span::new(BytePos::from_usize(12), BytePos::from_usize(13), source_id),
                    ),
                },
                FunctionArg {
                    ident: SpannedStr::new(
                    ...
                    ty: SpannedTypeAnnotation::new(
                        TypeAnnotation::Ident("d"),
                        Span::new(BytePos::from_usize(18), BytePos::from_usize(19), source_id),
                    ),
                },
            ]),
            ret_ty: SpannedTypeAnnotation::new(
                TypeAnnotation::Ident("e"),
                        ...
                        rhs: Box::new(SpannedExpr::new(
                            Expr::Binary {
                                lhs: Box::new(SpannedExpr::new(
                                    Expr::Ident("d"),
                                    Span::new(
                                        BytePos::from_usize(13),
                                        BytePos::from_usize(14),
                                        source_id,
                                    ),
                                )),
                                rhs: Box::new(SpannedExpr::new(
                                    Expr::Ident("e"),
                                    Span::new(
                                        BytePos::from_usize(17),
                                        BytePos::from_usize(18),
                                        source_id,
                                    ),
                                )),
                                op: BinaryOp::Sub,
...
fn stmt_block_expr_test() {
    ...
    let expected_stmts = [SpannedStmt::new(
        Stmt::TailExpr(SpannedExpr::new(
            Expr::Block(SpannedStmtBlock::new(
                Box::new([
                    SpannedStmt::new(
...
fn let_parse_test() {
    let (source_map, source_id) =
        generate_source("let a = a;let b: u8 = b;let c;let e: i8;let mut f;let mut g: f32;");
    let tokens = lex_source(&source_map, source_id);
    let parse_result = Parser::new(tokens, source_id, &source_map).parse_full();
...
#[test]
fn multilevel_tuple_type_annotation_parse_test() {
    let (source_map, source_id) = generate_source("(a, (b, c))");
    let tokens = lex_source(&source_map, source_id);
    let mut parser = Parser::new(tokens, source_id, &source_map);
...
#[test]
fn fully_defined_array_parse_test() {
    let (source_map, source_id) = generate_source("[a, b, c]");
    let tokens = lex_source(&source_map, source_id);
    let parse_result = Parser::new(tokens, source_id, &source_map).parse_full();
    ...
    assert_eq!(
        ...
        [SpannedStmt::new(
            Stmt::TailExpr(SpannedExpr::new(
                Expr::FullyDefinedArray(Box::new([
                    SpannedExpr::new(
                        Expr::Ident("a"),
                        Span::new(BytePos::from_usize(1), BytePos::from_usize(2), source_id)
                    ),
                ...
                Span::new(BytePos::from_usize(0), BytePos::from_usize(9), source_id)
            )),
            Span::new(BytePos::from_usize(0), BytePos::from_usize(9), source_id)
        )]
    );
...
fn attribute_access_parse_test() {
    ...
    assert_eq!(
        parse_result.stmts,
        [SpannedStmt::new(
            Stmt::TailExpr(attribute_access_expr(source_id)),
            Span::new(BytePos::from_usize(0), BytePos::from_usize(5), source_id)
        )]
    );
}
...
#[test]
fn attribute_access_call_parse_test() {
    let (source_map, source_id) = generate_source("a.b.c()");
    let tokens = lex_source(&source_map, source_id);
    let parse_result = Parser::new(tokens, source_id, &source_map).parse_full();
    ...
    assert_eq!(
        ...
        [SpannedStmt::new(
            Stmt::TailExpr(SpannedExpr::new(
                Expr::Call {
                    callee: Box::new(attribute_access_expr(source_id)),
                    args: Box::new([]),
                },
                Span::new(BytePos::from_usize(0), BytePos::from_usize(7), source_id)
            ),),
