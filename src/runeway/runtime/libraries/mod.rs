...
pub use module_type::{RNWModule, register as register_module};
...
pub fn load_library(import_path: String, working_dir: &Path) -> RWResult<EnvRef> {
    if let Some(name) = import_path.strip_prefix("std::") {
        ...
        Ok(loaded::get_loaded(&import_path).unwrap_or_else(
            || panic!("InternalError: Cannot load the library '{}'", import_path)))
    } else {
        ...
        if !path.is_file() {
            return Err(
                RuneWayError::new(RuneWayErrorKind::Runtime(Some("FileSystemError".to_string())))
                    .with_message(format!("Path is not a file or it does not exists: {}", path.display()))
            )
        }
        ...
        if !loaded::is_loaded(&import_path) {
            ...
            let parsed_code = parse_code(code.clone()).unwrap_or_else(
                |e|  {
            ...
            ASTInterpreter::execute_many(env.clone(), parsed_code.ast, working_dir, filename, &code)
                .map_err(|e| {
                    e.with_code_base(filename, &code)
                })?;
